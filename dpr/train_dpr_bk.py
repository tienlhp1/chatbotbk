...
from util import (
    get_tokenizer,
    build_dpr_traindata_bk,
    build_dpr_testdata_bk,
    embed_corpus,
)
...
def main():
    ...
    parser.add_argument(
        "--BE_score", default="dot", type=str, help="Type of similarity score"
    )
    parser.add_argument(
        "--load_path",
        default=None,
    ...
    parser.add_argument(
        "--q_len", default=32, type=int, help="Maximum token length for question"
    )
    parser.add_argument(
        "--ctx_len", default=256, type=int, help="Maximum token length for context"
    )
    parser.add_argument(
        "--BE_train_batch_size",
        default=1,
    ...
    parser.add_argument(
        "--BE_lr", default=0.00001, type=float, help="Biencoder training learning rate"
    )
    ...
    parser.add_argument(
        "--final_path", default=None, type=str, help="Path to save the final state"
    )
    parser.add_argument(
        "--biencoder_path",
        default=None,
    ...
    parser.add_argument(
        "--index_path", default=None, type=str, help="Path to save the index"
    )
    ...
    print("\t* Loading data...")
    print(args)
    val_loader = build_dpr_traindata_bk(
        df=dval,
        tokenizer=tokenizer,
        q_len=args.q_len,
    ...
    train_loader = build_dpr_traindata_bk(
        df=dtrain,
        tokenizer=tokenizer,
        q_len=args.q_len,
        ctx_len=args.ctx_len,
    ...
    dpr_retriever.test_on_data(top_k=[1, 5, 10, 30, 100], segmented=False)
