...
def preprocess_all_title(article_title):
    ...
    re_head = re.compile(
        r"(thông tư liên tịch|thông tư|nghị quyết|quyết định|nghị định|văn bản)\s(quy định|hướng dẫn)?"
    )
...
def build_corpus(f, corpus_file, scorpus_ids, head=False):
    """
    ...
    for law in data:
        for article in law["articles"]:
            ...
            processed_text = title + ". " + article_text
...
def build_short_corpus(f, corpus_file, head=False, size=200, overlap=64):
    """
    Build a corpus-dataframe
    """
    ids = []
        ...
        for article in law["articles"]:
            ...
            try:
                ...
                if tokenized_len <= size + 10:
                    ...
                    windows = create_sliding_window(
                        tokenized_text, size=224, overlap=64
                    )
                ...
                for i in range(n_big_windows):
                    big_windows.append(
                        "".join(
                            processed_text[i * actual_size : i * actual_size + size]
                        )
                    )
    ...
    if not split:
        for item in data["items"]:
            ...
            for i in range(len(item["relevant_articles"])):
                ans_count += 1
                atext_id = (
                    item["relevant_articles"][i]["law_id"]
                    + "_"
                    + item["relevant_articles"][i]["article_id"]
                )
        ...
        for item in data["items"]:
            ...
            for article in item["relevant_articles"]:
                q_texts.append(question)
                q_processed_text = preprocess_question(
                    question, remove_end_phrase=False
                )
                q_processed_texts.append(q_processed_text)
                q_tokenized_text = tokenise(q_processed_text, f)
                q_tokenized_texts.append(q_tokenized_text)
...
def build_short_data(df, dcorpus, limited_length=234):
    ...
    """
    ids = [
        i for i in range(len(df)) if dcorpus["len"][df["ans_id"][i]] <= limited_length
    ]
